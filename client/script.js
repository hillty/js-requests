...
const balls = 'i-am-bad-at-coding'

const repeatMyParam = () => {
    axios.get(`http://localhost:3000/repeat/${balls}`).then(
        (req, res) => {
            let lolz = document.querySelector('#repeat-text')
            console.log(req.data)
            lolz.textContent = req.data
            console.log(lolz)
        })
    }
...
let buttCheek = 'this=sucks'
function queryRequest(){
    axios.get(`http://localhost:3000/query-test/?${buttCheek}`).then(
        (req, res) => {
            console.log(req.data)
        }
    )
